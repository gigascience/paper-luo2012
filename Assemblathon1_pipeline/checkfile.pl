...
if($step ne "filter" && $step ne "kmerfreq" && $step ne "correct" && $step ne "pregraph" && $step ne "contig" && $step ne "map" && $step ne "scaff" && $step ne "all" && $step ne "kgf" && $step ne "gapcloser" && $step ne "filtercont" && $step ne "consensus")
{
    print "There is no step $step\n";
    exit -1;
}

if($step eq "pregraph" && $step eq "contig" && $step eq "map" && $step eq "scaff" && $step eq "all" && $step eq "kgf" && $step eq "gapcloser" && $step eq "consensus")
{
...
if($step eq "kgf")
{
    if( (!-s "../kgf/SRkgf/step_2/$prefix.scafSeq.fill" && !-s "SRkgf/step_2/$prefix.scafSeq.fill" && !-s "$prefix.scafSeq.fill") )
    {
	print "Missing file in step kgf\n";
	exit -1;
    }
}
...
if($step eq "filtercont")
{
    if( (!-s "../filterContamination/$prefix.scafSeq.GC.filter " && !-s "$prefix.scafSeq.GC.filter") )
    {
	print "Missing file in step $step\n";
	exit -1;
    }
...
{
    if( (!-s "../consensus/$prefix.scafSeq.GC.filter.consensus" && !-s "$prefix.scafSeq.GC.filter.consensus") )
    {
        print "Missing file in step $step\n";
        exit -1;
    }
